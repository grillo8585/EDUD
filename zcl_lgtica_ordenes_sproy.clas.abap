class ZCL_LGTICA_ORDENES_SPROY definition
  public
  inheriting from ZCL_LGTICA_DOCUMENTO
  final
  create public .

*"* public components of class ZCL_LGTICA_ORDENES_SPROY
...
CLASS ZCL_LGTICA_ORDENES_SPROY IMPLEMENTATION.
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Entregas para Proceso Ordenes de servicio Proyectos
* Autor Prog.  :
...
*.....Field Simbols para Procesamiento de Información de ventas (Ordenes de servicio Proyectos)
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Cabecera de Documentos para Ordenes de servicio Proyectos
* Autor Prog.  :
...
*.....Typo para Tabla VBUP, Pedidos de Venta (Ordenes de servicio Proyectos) Pendiente por Picking
...
*.....Typo para Detalle Cabecera Pedido de Ventas (Ordenes de servicio Proyectos) VBAK
...
*.....Busqueda de documentos Pendientes por Picking para Pedidos de Venta (Ordenes de servicio Proyectos)
...
*.....Busco que el Número de Pedido de Ventas este Pendiente por Picking
        READ TABLE lti_vbup WITH KEY vbeln = i_documento_origen
                            ASSIGNING <lfs_vbup>.
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Detalle de Documentos Ordenes de servicio Proyectos
* Autor Prog.  :
* Fecha Creac. : 24.06.2014
*-------------------------------------------------------------------------------*
* Ordenes de Transporte
...
*.....Typo para Busqueda Documento de Ventas (Ordenes de servicio Proyectos)  Pendiente por Picking VBUP
  TYPES : BEGIN OF ltp_vbup,
            vbeln TYPE vbeln,       "Número de documento comercial
            posnr TYPE posnr,       "Número de posición del documento comercial
            lfgsa TYPE 	lfgsa,      "Status total de entrega de posición
...
*\*.....Typo para Busqueda Detalle Posiciones del Documento Pedido de Ventas (Ordenes de servicio Proyectos) VBAP
*\  TYPES : BEGIN OF ltp_vbap,
...
*}   REPLACE
  ...
  DATA :
*.....Tabla Interna para Busqueda de Pedidos de Venta Pendientes (Ordenes de servicio Proyectos) por Picking
           lti_vbup TYPE TABLE OF ltp_vbup,
*.....Tabla Interna de Posiciones de Pedidos de Venta (Ordenes de servicio Proyectos)
           lti_vbap TYPE TABLE OF ltp_vbap,
*.....Tabla Interna para Cabecera Pedidos de Venta (Ordenes de servicio Proyectos)
           lti_vbak TYPE TABLE OF ltp_vbak,
...
*.....Busqueda de Documentos de Pedidos de Ventas (Ordenes de servicio Proyectos) Pendientes por Picking
...
*.....Ordeno la Tabla para el For All Entries
        SORT lti_vbap BY  vbeln posnr ASCENDING.
*.....Busco Pedidos de Venta (Ordenes de servicio Proyectos)  Pendientes por Picking
        SELECT vbeln posnr lfgsa
            FROM vbup
            INTO TABLE lti_vbup
               FOR ALL ENTRIES IN lti_vbap
                 WHERE vbeln EQ lti_vbap-vbeln AND
                       posnr EQ lti_vbap-posnr.
*.....Verifico que Existan Registros
        IF sy-subrc EQ 0.
...
*\          ENDLOOP.
*z          LOOP AT  lti_vbak ASSIGNING <lfs_vbak>  .
            DELETE lti_vbak WHERE auart NE <lfs_actividad>-cod_clase.
*          ENDLOOP.
*}   REPLACE
        ELSE.
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Obtener la Ubicación sugerida de (Ordenes de servicio Proyectos)
* Autor Prog.  :
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Carga los datos del documento (Ordenes de servicio Proyectos)
* Autor Prog.  :
...
*-------------------------------------------------------------------------------*
*.....Metodo para Setiar Atributos de ventas (Ordenes de servicio Proyectos)
  call method me->zif_lgtica_documento~set_ventas
    exporting
      i_clase_documento  = i_clase_documento
      i_documento_origen = i_documento_origen.
...
*.....Indicador para Proceso que Genera Entrega (Esta Vacio, se debe hacer consumo de Inventario MIGO)
   c_indicador = ' '.
endmethod.

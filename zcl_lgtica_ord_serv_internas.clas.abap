class ZCL_LGTICA_ORD_SERV_INTERNAS definition
  public
  inheriting from ZCL_LGTICA_DOCUMENTO
  final
  create public .

*"* public components of class ZCL_LGTICA_ORD_SERV_INTERNAS
...
public section.
  ...
  methods ZIF_LGTICA_DOCUMENTO~GET_DETALLE_PACKING
    redefinition .
protected section.
*"* protected components of class ZCL_LGTICA_ORD_SERV_INTERNAS
*"* do not include other source files here!!!
private section.
*"* private components of class ZCL_LGTICA_ORD_SERV_INTERNAS
*"* do not include other source files here!!!
ENDCLASS.



CLASS ZCL_LGTICA_ORD_SERV_INTERNAS IMPLEMENTATION.
...
*      les_msg_picking-msg = l_e_msg.
      ...
      RETURN.
  ENDIF.
*     READ TABLE detalles_reservas into les_reservas_dt INDEX 1.
*.... Ajusto el destinatario de mercancías
      CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
        EXPORTING
          input         = les_resb-wempf
       IMPORTING
         OUTPUT        = l_s_KUNWE
               .
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Cabecera de Documentos para Ordenes de Servicio Internas
* Autor Prog.  : Marco Suarez
* Fecha Creac. : 11.08.2014
*-------------------------------------------------------------------------------*
* Ordenes de Transporte
*-------------------------------------------------------------------------------*
* Fecha       | CR#         | Autor           | Modificación
*-------------------------------------------------------------------------------*
* 11.08.2014    ER6K907162    Marco Suarez        Creación
*-------------------------------------------------------------------------------*
*{   INSERT         ER3K900279                                        1
...
data: les_ubicacion_suge type  ZESD_UBIC_SUG.
  ...
  FIELD-SYMBOLS : <lfs_return> TYPE bapiret1,
                  ...
                  <lfs_aufk> TYPE ltp_aufk,
                  <lfs_rkpf> TYPE ltp_rkpf,
                  <lfs_res_items> TYPE ltp_reservas.
*}   REPLACE
...
*\                      WHERE sowrk EQ detalles_usuario-centro AND
...
*\                        auart EQ i_clase_documento.            "Clase de Orden
                  WHERE   auart EQ i_clase_documento.
*}   REPLACE
      ENDIF.
...
*......Verifico que Existan Ordenes con los Rangos de Fecha Ingresados
            IF lti_aufk[] IS INITIAL .
              les_msg-msg = 'Para el Rango de Fechas Ingresados no Existen Ordenes de Servicio'.
              les_msg-type_msg = 'E'.
              APPEND les_msg TO c_mensajes.
              RETURN.
            ELSE.
...
*                    les_cab_ref-cod_cliente =
                    les_cab_ref-nom_cliente = <lfs_aufk>-ktext.   "Texto breve
...
*            les_msg-num_doc = i_documento_origen.
                ...
                ENDIF.
              ELSE.
                CONCATENATE 'No Existen Componentes y/o Reservas para las Ordenes de Servicio del Usuario'  space
...
*.....Recorro Tablas de Reserva para Identificar si existe almenos 1 Posicion pendiente por Picking
                LOOP AT lti_res_items ASSIGNING <lfs_res_items>.
*{   INSERT         ER3K900279                                        5
                   APPEND <lfs_res_items>-MATERIAL TO lti_matnr.
*}   INSERT
                  IF <lfs_res_items>-movement EQ 'X'.
                    EXIT.
                  ELSE.
...
*   y lo llevamos a la tabla que se exportará
    ...
    WHERE centro   EQ lti_res_items-PLANT
      AND almacen  EQ lti_res_items-STGE_LOC
      AND material EQ lti_res_items-MATERIAL.


ENDIF.

*.....Solo Recorro las Posiciones Pendientes por Picking
              LOOP AT lti_res_items ASSIGNING <lfs_res_items> WHERE movement = 'X'.

**     INICIO SO-126 LFY 07/04/2014  "se agrega una condición al WITH KEY (kzear = '')
*
...
*... Convierto la cantidad total de la reserva y la convierto a UMB
                    READ TABLE lti_detalle assigning <lfs_detalle> WITH KEY matnr = les_resb-matnr unidad_medida = les_resb-erfme .
                    IF sy-subrc EQ 0.
*....Asigno el calculo de la unidad de medida base y la cantidad si la encuentra
                      IF les_resb IS NOT INITIAL.
...
*            les_cab_ref-tipo_doc    = <lfs_vbak>-auart.
                  les_cab_ref-cod_cliente = <lfs_resb>-wempf.
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Detalle para Ordenes de Servicio Internas
* Autor Prog.  : Marco Suarez
* Fecha Creac. : 12.08.2014
*-------------------------------------------------------------------------------*
* Ordenes de Transporte
*-------------------------------------------------------------------------------*
* Fecha       | CR#         | Autor           | Modificación
*-------------------------------------------------------------------------------*
* 12.08.2014     ER6K907163    Marco Suarez        Creación
*-------------------------------------------------------------------------------*

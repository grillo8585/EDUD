class ZCL_LGTICA_PEDIDOS_VENTA definition
  public
  inheriting from ZCL_LGTICA_DOCUMENTO
  final
  create public .

*"* public components of class ZCL_LGTICA_PEDIDOS_VENTA
...
public section.
  ...
  methods ZIF_LGTICA_PICKABLE~GET_POS_PICKING
    redefinition .
protected section.
*"* protected components of class ZCL_LGTICA_PEDIDOS_VENTA
*"* do not include other source files here!!!
private section.
...
CLASS ZCL_LGTICA_PEDIDOS_VENTA IMPLEMENTATION.
...
*{   INSERT         ER3K900309                                       10
      DELETE ADJACENT DUPLICATES FROM detalles_ventas COMPARING werks.
      DELETE detalles_ventas  where werks NE detalles_usuario-centro.
*}   INSERT
...
*\      CALL METHOD zcl_lgtica_picking=>create_by_pkble_ref
      CALL METHOD zcl_lgtica_picking=>create_by_pkble_ref_pvta
*}   REPLACE
        EXPORTING
          i_ref_pickable = lo_pick.
*.....Actualizo Base de Datos
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Cabecera de Documentos para Pedidos de Venta
* Autor Prog.  : Marco Suarez
...
*-------------------------------------------------------------------------------*
*{   INSERT         ER3K900279                                        2
*.... Espacio para el manejo del detalle
...
*.....Typo para Tabla VBUK
  TYPES : BEGIN OF ltp_vbuk,
          vbeln TYPE vbeln,       "Número de documento comercial
          CMGST type CMGST,
    END OF ltp_vbuk.
...
*           lti_vbup TYPE TABLE OF ltp_vbup,
           les_vbup TYPE  ltp_vbup,
*.....Tabla Interna para Busqueda de Pedidos de Venta Pendientes por Picking cantidad confirmada
           lti_vbep TYPE TABLE OF vbep,
           les_vbep TYPE  vbep,
*.....Tabla Interna de Posiciones de Pedidos de Venta
*           lti_vbap TYPE TABLE OF ltp_vbap,
           les_vbap TYPE  ltp_vbap,
...
*.....Tabla Interna del Tipo ltp_vbup
        lti_vbup TYPE TABLE OF ltp_vbup,
*.....Tabla para estados Pedido de Ventas
        lti_vbuk TYPE TABLE OF ltp_vbuk,
        les_vbuk TYPE ltp_vbuk,
...
*.....Función para Ajuste de Número
        CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
          ...
          IMPORTING
            output = lv_vbeln.
*.... Validar pedidos bloqueados por cartera
        SELECT vbeln CMGST
               FROM vbuk
               INTO TABLE lti_vbuk
               WHERE vbeln EQ lv_vbeln.
        LOOP AT lti_vbuk into les_vbuk.
          IF les_vbuk-CMGST EQ 'B'.
          DELETE lti_vbuk index sy-tabix.
          les_msg-msg = 'Documento Bloqueado por cartera'.
          les_msg-type_msg = 'E'.
          APPEND les_msg TO c_mensajes.
          RETURN.
          ENDIF.
...
*{   INSERT         ER3K900279                                        1
*.... Verifico el Flag para la consulta del detalle
          IF I_FLAG_DET EQ 'X'.
*.....Consulto Tabla de Posiciones con su cantidad confirmada
        SELECT MANDT VBELN POSNR ETENR ETTYP LFREL EDATU EZEIT WMENG BMENG VRKME LMENG MEINS BDDAT BDART PLART
               VBELE POSNE ETENE RSDAT IDNNR BANFN BSART BSTYP WEPOS REPOS LRGDT PRGRS TDDAT MBDAT LDDAT WADAT
               CMENG LIFSP GRSTR ABART ABRUF ROMS1 ROMS2 ROMS3 ROMEI RFORM UMVKZ UMVKN VERFP BWART BNFPO ETART
               AUFNR PLNUM SERNR AESKD ABGES MBUHR TDUHR LDUHR WAUHR AULWE
          FROM vbep
          INTO TABLE lti_vbep
          FOR ALL ENTRIES IN lti_vbap
          WHERE vbeln EQ lti_vbap-vbeln AND
...
*.....Busco el Codigo de la Clase para El documento Recibido
        READ TABLE lti_actividad WITH KEY cod_clase = les_vbak-auart ASSIGNING <lfs_actividad>.
        IF sy-subrc EQ 0 .
...
*  .....Función para Obtener EANs de un Material
          CALL FUNCTION 'Z_SD_CONSUL_EAN_MATE'
            TABLES
              ...
              t_mara = lti_mara.
          ENDIF.
*.... Ordeno tabla de posiciones
      SORT lti_vbap by VBELN MATNR ASCENDING.

       LOOP AT lti_vbap ASSIGNING <lfs_vbap> WHERE  vbeln = <lfs_vbup>-vbeln.
            CLEAR: les_vbep.
*.....Busco Registros que esten Pendiente por Picking con el Número Doc Recibido
            LOOP AT lti_vbep INTO les_vbep where vbeln = lv_vbeln and posnr = <lfs_vbap>-posnr and EDATU <= sy-datum.

*            READ TABLE lti_vbep WITH KEY vbeln = lv_vbeln  posnr = <lfs_vbap>-posnr INTO les_vbep.
...
*.....Busco Registros que esten Pendiente por Picking con el Número Doc Recibido
            READ TABLE lti_vbup WITH KEY vbeln = lv_vbeln  posnr = <lfs_vbap>-posnr INTO les_vbup.
...
*.... Convierto a unidad de medida base
                      IF les_vbep-VRKME NE les_vbep-MEINS.
                          les_vbep-bmeng = ( les_vbep-bmeng * les_vbep-UMVKZ ) / les_vbep-UMVKN.
                      ENDIF.
                      les_det_pos-cant_por_pick = les_vbep-bmeng.
...
*.....Realizar la conversión de la unidad de medida
                      CALL FUNCTION 'CONVERSION_EXIT_CUNIT_OUTPUT'
                        ...
                        EXCEPTIONS
                          ...
                          OTHERS         = 2.
                      IF l_e_meinh EQ '004'.
                        les_det_pos-uni_med_doc = les_vbep-MEINS.
                      ELSE.
...
* Información General
*-------------------------------------------------------------------------------*
* Descripción  : Detalle de Documentos Pedidos de Ventas
* Autor Prog.  : Marco Suarez
* Fecha Creac. : 24.06.2014
*-------------------------------------------------------------------------------*
* Ordenes de Transporte
...
*.....Typo para Busqueda Documento de Ventas Pendiente por Picking VBUP
  TYPES : BEGIN OF ltp_vbup,
            ...
            posnr TYPE posnr,       "Número de posición del documento comercial
            lfgsa TYPE 	lfgsa,      "Status total de entrega de posición
            gbsta TYPE  gbsta,      "Status de tratamiento global p.pos.doc.comercial
      END OF ltp_vbup.
...
*   INICIO SO-168 LFY 04/06/2015
*.....Busqueda de Documentos de Pedidos de Ventas Pendientes por Picking
*      SELECT vbeln posnr matnr arktx kwmeng  vrkme werks lgort "Se quitan los filtros de centro y almacen
*           FROM  vbap
*            INTO CORRESPONDING FIELDS OF TABLE lti_vbap
*              WHERE vbeln EQ lv_vbeln.
...
*.....Busco Pedidos de Venta Pendientes por Picking
        SELECT vbeln posnr lfgsa gbsta
            FROM vbup
            INTO TABLE lti_vbup
               FOR ALL ENTRIES IN lti_vbap
                 WHERE vbeln EQ lti_vbap-vbeln AND
...
**        IF sy-subrc EQ 0.
**          UNASSIGN: <lfs_vbup>.
***          Si existen posiciones pendientes de picking y no son del mismo centro y almacen del usuario
***          se eliminan de lti_vbap y lti_vbup
**          LOOP AT lti_vbup ASSIGNING  <lfs_vbup> WHERE gbsta NE 'C'.
**            READ TABLE lti_vbap INTO les_vbap WITH KEY posnr = <lfs_vbup>-posnr.
**            CHECK sy-subrc IS INITIAL.
**            IF les_vbap-werks NE detalles_usuario-centro
**              AND les_vbap-lgort NE detalles_usuario-almacen.
**              DELETE lti_vbap WHERE posnr = <lfs_vbup>-posnr.
**              DELETE lti_vbup WHERE posnr = <lfs_vbup>-posnr.
**            ENDIF.
**
**          ENDLOOP.
**        ENDIF.
*   FIN SO-168 LFY 04/06/2015
      ELSE.
        les_msg-num_doc = i_documento_origen.
        les_msg-msg = 'Usuario No Autorizado para Centro y/o Almacen'.
        les_msg-type_msg = 'E'.
...
*\          les_pos_picking-UMC = les_lips-VRKME. " unidad de medida venta
          ...
          les_pos_picking-UMC = les_lips-MEINS. " unidad de medida venta
*}   REPLACE
